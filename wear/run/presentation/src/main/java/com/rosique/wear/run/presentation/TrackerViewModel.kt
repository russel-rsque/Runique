...
class TrackerViewModel(
    ...
    init {
        ...
        isAmbientMode
            .flatMapLatest { isAmbientMode ->
                if (isAmbientMode) {
                    ...
                    runningTracker.elapsedDuration
                }
            }
            .onEach {
        ...
        runningTracker
            .distanceMeters
            .onEach {
                state = state.copy(distanceInMeters = it)
            }
            .launchIn(viewModelScope)
    ...
    private fun listenToPhoneActions() {
        phoneConnector
            ...
            .onEach { action ->
                when(action) {
                    is MessagingAction.Finish -> {
                        onAction(TrackerAction.OnFinishRunClick, true)
                    }

                    is MessagingAction.Pause -> {
                        if (state.isTrackable) {
                            state = state.copy(isRunActive = false)
                        }
                    }

                    is MessagingAction.StartOrResume -> {
                        if (state.isTrackable) {
                    ...
                    is MessagingAction.NotTrackable -> {
